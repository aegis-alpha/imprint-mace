---
name: imprint-memory
description: Persistent memory for AI agents via Imprint MCP. Use when you need to remember facts across sessions, recall past decisions, or explore the knowledge graph.
---

# Imprint Memory

Imprint is a persistent knowledge graph. It stores facts, entities, and relationships extracted from conversations. Use it to remember things across sessions and recall them later.
...
- To change fact content (use `imprint_supersede_fact` instead)
- To adjust confidence without a reason
...
- If the fact is old but still true (old facts are not wrong facts)
- To delete a fact (superseding preserves history; there is no delete)
...
```
imprint_entities(type: "project")           -- list all projects
imprint_graph(entity: "Acme", depth: 2)     -- see Acme's connections
imprint_query(question: "What tech stack does Acme use?")  -- synthesized answer
```
...
```
imprint_query(question: "What language does Alice use?")
-- answer cites fact 01JFA... saying "Alice uses Rust"
-- but you know Alice switched to Go
