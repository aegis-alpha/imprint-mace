...
Seven MCP tools. All names start with `imprint_`.
...
```
imprint_ingest text="Alice prefers dark mode in all editors. Decided to use Go for the Acme project." source="session-2026-03-15"
```
...
Questions in English or Russian both work.
...
Call at session start to verify the knowledge base is available. Call when the user asks "how much do you remember?" or "what's in your memory?"
...
Call when you need an overview: "What projects exist?" -> `imprint_entities type="project"`. "Who is involved?" -> `imprint_entities type="person"`.
...
Call when you need to understand connections: "Who works on Acme?" -> `imprint_graph entity="Acme"`. Combine with `imprint_entities` for navigation: list entities first, then graph a specific one.
...
- Fact has wrong confidence ("this is certain, not 0.5" -> set to 0.95)
- Fact became time-limited ("this is only true until end of March" -> set valid_until)
- Subject is wrong ("this is about Acme, not DataSync" -> correct subject)

Do NOT call to change fact content (use `imprint_supersede_fact` instead).

```
imprint_update_fact fact_id="01JFA..." confidence=0.95
imprint_update_fact fact_id="01JFA..." valid_until="2026-04-01T00:00:00Z"
```
...
- Fact content is outdated ("Alice now uses Go, not Rust")
- Fact was inaccurate ("deploys are on Thursdays, not Fridays")
- Information changed ("the API moved from REST to gRPC")

Do NOT call if the fact is old but still true. Do NOT use to delete facts (superseding preserves history).

```
imprint_supersede_fact old_fact_id="01JFA..." new_content="Alice switched to Go in March 2026." source="session-2026-03-15"
```
...
1. `imprint_status` -- confirms the knowledge base is reachable
2. `imprint_query question="What is the current context for this project?"` -- loads relevant context (this is the mandatory auto-query -- see above)
...
```
imprint_ingest text="Decided to switch from REST to gRPC for the internal API. Reason: need bidirectional streaming." source="session-2026-03-15"
```

### Exploring the knowledge graph

1. `imprint_entities type="project"` -- list all projects
2. `imprint_graph entity="Acme" depth=2` -- see Acme's connections
3. `imprint_query question="What tech stack does Acme use?"` -- synthesized answer

## Do NOT

- Call `imprint_ingest` after every message. Only when there is knowledge to store.
- Skip the mandatory auto-query (see MANDATORY: Auto-Query above). Call additional targeted queries when you need specific facts: entity details, graph connections, historical decisions.
